const SafeStorage = {
  ...
  get: (key, fallback = "[]") => {
    try {
      return JSON.parse(
        localStorage.getItem(SafeStorage.prefix + key) || fallback,
      );
    } catch (e) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const defaultTabs = [
    ...
    },
    {
      url: "https://4chan.org",
      name: "4chan",
      // icon: '<svg viewBox="0 0 24 24" fill="currentColor"><rect width="24" height="24" rx="2" /><path d="M13.44 13.91V20h-2.8v-6.09L6.15 4h3.18l2.67 6.13L14.67 4h3.18z" fill="#000" /></svg>',
    },
  ];
  ...
  const trackVisit = (url, name) => {
    ...
    const existingIndex = history.findIndex(
      (i) => normalizeDeep(i.url) === normKey,
    );
    if (existingIndex !== -1) {
      history[existingIndex].count++;
      history[existingIndex].lastVisit = Date.now();
      if (url.length > history[existingIndex].url.length)
  ...
  const renderTray = () => {
    ...
    if (items.length < 4) {
      defaultTabs.forEach((def) => {
        if (
          !items.find((i) => normalizeDeep(i.url) === normalizeDeep(def.url)) &&
          items.length < 4
        ) {
    ...
    const trayItems = items.slice(0, 4).map((item) => {
      const defTab = defaultTabs.find(
        (d) => normalizeDeep(d.url) === normalizeDeep(item.url),
      );
    ...
    if (trayItems.length === 0) {
      ...
      emptyMsg.className =
        "flex-1 flex items-center justify-center py-4 text-[10px] text-white/20 tracking-[0.2em] uppercase";
      emptyMsg.innerHTML = `
    ...
    trayItems.forEach((item) => {
      ...
      if (item.icon) {
        ...
        svgWrapper.className =
          "flex items-center justify-center icon-shortcut transition-all";
        ...
        img.src = `https://www.google.com/s2/favicons?domain=${domain}&sz=64`;
        img.className =
          "rounded-full shadow-md opacity-0 transition-opacity duration-300 pointer-events-none icon-shortcut";
        ...
        img.onerror = () => {
          ...
          fallback.className =
            "w-5 h-5 rounded-full bg-white/10 flex items-center justify-center text-[8px] uppercase font-bold";
  ...
  const renderSuggestions = (suggestions, isBangs = false) => {
    ...
    suggestions.forEach((item, index) => {
      ...
      if (isBangs) {
        ...
        Object.assign(triggerSpan.style, {
    ...
    state.suggestionElements = Array.from(
  ...
  searchInput.addEventListener("input", (e) => {
    ...
    if (query.startsWith("!") && !rawValue.includes(" ")) {
      const bang = CONFIG.bangsList.find(
        (b) => b.trigger === query.toLowerCase(),
      );
      if (bang) {
        const hasSiblings = CONFIG.bangsList.some(
          (b) =>
            b.trigger.startsWith(query.toLowerCase()) &&
            b.trigger !== query.toLowerCase(),
        );
        if (!hasSiblings) {
  ...
  searchInput.addEventListener("keydown", (e) => {
    ...
    if (e.key === "ArrowDown" || (e.key === "Tab" && !e.shiftKey)) {
      e.preventDefault();
      const nextIdx =
        state.selectedIndex < items.length - 1 ? state.selectedIndex + 1 : 0;
      updateSelection(nextIdx);
      searchInput.value = items[nextIdx].dataset.value;
    } else if (e.key === "ArrowUp" || (e.key === "Tab" && e.shiftKey)) {
      e.preventDefault();
      const prevIdx =
        state.selectedIndex > 0 ? state.selectedIndex - 1 : items.length - 1;
  ...
  searchForm.addEventListener("submit", (e) => {
    ...
    if (inputVal.startsWith("!")) {
      ...
      const urlPattern =
        /^(https?:\/\/)?((localhost|[\w\-]+(\.[\w\-]+)+)|(\d{1,3}\.){3}\d{1,3})(:\d+)?(\/[\w\-\.\/?%&@=#+~]*)?$/i;
