...
export function getSavedSearchEngine() {
...
export function clearSearchInput(inputElement) {
    inputElement.value = '';
    inputElement.focus();
...
export async function loadShortcuts() {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error('Failed to retrieve remote shortcuts yaml file');
...
export function updateSearchSuggestions(inputVal) {
    ...
    const inputElement = document.getElementById('search-input');
    if (!ghostElement || !suggestionsElement)
        return;
...
export function handleSearchKeyDown(event, inputElement) {
    if (currentFilteredSuggestions.length === 0)
        return false;
